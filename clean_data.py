...
output_file = "filtered_structure2.json"
...
regex_patterns_to_remove = [
    ...
    re.compile(r"blog/(201[0-9]|202[0-5])/\d{2}/"),  # Excluye blog/YYYY/MM/ con años entre 2010-2025
    re.compile(r"blog/\?p=\d+"),  # Excluye URLs como blog/?p=1173
    re.compile(r"/(19[0-9]{2}|20[0-9]{2})/\d{2}/\d{2}/"),  # Excluye /YYYY/MM/DD/ desde 1900 en adelante
]

# Función para filtrar URLs
def is_valid_url(entry):
    url = entry.get("url", "")
    return not (any(keyword in url for keyword in keywords_to_remove) or any(pattern.search(url) for pattern in regex_patterns_to_remove))
